...
%% 1
fid = fopen('3an.raw','r'); % Se abre la señal
x = fread(fid,inf,'int16');

lenX = length(x); % Se obtiene el tamaño de la señal, indispensable para usos futuros

n = 128; % Número de muestras por ventana
M = n;
nV = lenX/n; % Número de ventanas
...
% Graficación de las líneas horizontales del punto 4
...
% Cálculo de le para el fin de palabra
lef = nV;
...
% Cálculo de lz para el fin de palabra
lzf = lef;
...
% Graficación de líneas horizontales del punto 5
figure('name','Señal 1');
subplot(131);
plot(Mx);
hold;
...
%% Grafica de Señal original y recortada

xRecortada = x((lz-1)*M:lzf*M);

figure('name','Señal Original y recortada')
subplot(121);
plot(x);
